...
Run this file, then open http://localhost:5000
"""
...
import yt_dlp, static_ffmpeg, shutil, json, hashlib, datetime, threading, uuid, re, time
...
# In-memory download jobs: {job_id: {status, progress, speed, eta, log, error}}
_jobs = {}
...
def register():
    ...
    accs[email] = {"name":name, "pw_hash":hash_pw(pw),
                   "joined":datetime.date.today().isoformat(), "method":"email"}
...
def google_login():
    ...
    if email not in accs:
        ...
        accs[email] = {"name":gname, "pw_hash":"",
                       "joined":datetime.date.today().isoformat(), "method":"google"}
        jsave(ACCS_FILE, accs)
    u = accs[email]
...
def start_download():
    ...
    path    = d.get("path", os.path.join(os.path.expanduser("~"),"Downloads"))
    user    = d.get("user","")
    ...
    _jobs[job_id] = {"status":"starting","progress":0,"speed":"—","eta":"—",
    ...
    def run():
        ...
        try:
            ...
            job["progress"] = 100
            job["done"]     = True
            job["log"].append({"text":"✔  Download complete — saved to: "+path, "type":"success"})

            hist = jload(HIST_FILE, [])
            hist.append({"user":user,"url":url,"title":title,"mode":mode,
...
def update_ytdlp():
    try:
        ...
        return jsonify(ok=True, message="yt-dlp updated successfully!")
    except Exception as e:
        return jsonify(ok=False, message=str(e))

if __name__ == "__main__":
    import webbrowser
    print("\n  ▶  YouTube Downloader is starting…")
